...
from .utils import get_idf_dict, bert_cos_score_idf, get_bert_embedding, bert_types
...
def plot_example(
    h, r, verbose=False, bert="bert-base-multilingual-cased", num_layers=8, fname=""
):
    """
    BERTScore metric.
    Args:
    ...
    ref_embedding, ref_lens, ref_masks, padded_idf = get_bert_embedding(
        [r], model, tokenizer, idf_dict, device=device
    )
    hyp_embedding, ref_lens, ref_masks, padded_idf = get_bert_embedding(
        [h], model, tokenizer, idf_dict, device=device
    )
